...
Три способа наполнить очередь импорта ДО запуска обработки:
  - ручной путь к папке (было и раньше);
...
оно стало доступно в «Инспекторе панорамы» и «Активном обучении».
"""
...
st.caption(
    "Соберите очередь изображений (путь / папка / отдельные файлы), проверьте "
    "её, затем запустите обработку. Результаты попадают в «Историю и лог»."
)
...
# --- Наполнение очереди: три источника (вкладки) ----------------------------
tab_path, tab_folder, tab_files = st.tabs([
    "📁 Ручной путь к папке", "🗂️ Выбрать папку (проводник)", "🖼️ Выбрать файлы (проводник)",
])

with tab_path:
    default_folder = str(config.UPLOADS_DIR)
    folder_str = st.text_input(
        "Папка с изображениями", value=default_folder,
        help="Абсолютный или относительный путь. Ищет рекурсивно, включая подпапки.",
    )
    if st.button("Сканировать папку", key="scan_manual_path"):
        folder = Path(folder_str.strip()) if folder_str.strip() else None
        if not folder or not folder.is_dir():
            st.error(f"Папка не найдена: {folder}")
        else:
            items = bi.build_queue_from_folder(folder, source=bi.SOURCE_MANUAL_PATH)
            added = _add_items(items)
            ev.log_import(dataset_id, "queue_scan_path", folder=str(folder), found=len(items), added=added)
            st.success(f"Найдено {len(items)} изображений, добавлено в очередь: {added}.")

with tab_folder:
    st.caption(
        "Откроется системный диалог выбора папки. Поддерживаемые форматы "
        f"ищутся рекурсивно: {', '.join(config.SUPPORTED_FORMATS)}. Для очень "
        "больших наборов (десятки ГБ) удобнее вкладка «Ручной путь»."
    )
...
register_in_dataset = st.checkbox(
    "Также зарегистрировать изображения в датасете (для «Инспектора панорамы»)",
    value=True,
)
    ...
    if rows:
        ...
        if register_in_dataset:
            st.caption(
                ...
                "откройте «Инспектор панорамы», чтобы выбрать участок."
            )
