...
Раскладка на диске (см. docs/coordination/HANDOFF.md и README для описания):
  ...
  data/datasets/<dataset_id>/
    ...
    exports/active_learning_s2v2/<export_id>/    — тот же экспорт, второй ВАРИАНТ
      imgs/ masks/ masks_colored/ masks_human/    структуры (см. src/dataset_export.py,
                                                    формат «как в S2_v2»)
...
def export_active_learning_s2_style(
    dataset_id: str,
    *,
    statuses: tuple[str, ...] = ac.EXPORTABLE_STATUSES,
    export_id: Optional[str] = None,
    ...
    """
    Второй ВАРИАНТ сохранения того же набора разметок (не заменяет
    `export_active_learning`, а дополняет его) — структура папок «как в
    S2_v2» (см. src/dataset_export.py): imgs/ + masks/ + masks_colored/
    (+ masks_human/), файлы по sample_id. Лежит в отдельной ветке
    exports/active_learning_s2v2/<export_id>/, чтобы не пересекаться по именам
    папок с обычным экспортом (там images/, здесь imgs/).
    """
    ...
    class_names = {c.id: c.name_ru for c in ac.load_classes()}
        ...
        for image_dir in sorted(ann_root.iterdir()):
            ...
            for roi in list_rois(dataset_id, image_id):
                ...
                if mask is None or roi_image is None:
                    continue
                items.append({
                    "name": f"{image_id}__{region_id}",
                    "image": roi_image,
                    "mask": mask,
                })

    result = de.export_s2_bundle(edir, items, class_colors, class_names, include_human=include_human)
...
def list_exports_s2_style(dataset_id: str) -> list[str]:
    root = dataset_dir(dataset_id) / "exports" / "active_learning_s2v2"
    if not root.exists():
        return []
    return sorted(p.name for p in root.iterdir() if p.is_dir())
