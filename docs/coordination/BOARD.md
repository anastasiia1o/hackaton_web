...
- [DONE] Фронтенд-переработка: app.py→OreVision.py, «Активное обучение»/«История
  образцов», аннотатор с зумом+лассо (ui/annotator_frontend), экспорт→активное
  обучение порядок, снят лимит размера фото, фикс OOM батча + убран кэш по имени
  датасета, робастный сэмплинг (вырожденные случаи). 91 тест — stream-a/frontend-rework
