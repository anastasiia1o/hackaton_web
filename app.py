...
with st.expander("🔬 Инспектор участка (высокое разрешение)"):
    st.caption(
        "Обзор ниже уменьшен для скорости. Перетащите область мышью (потяните "
        "за уголок или край, чтобы изменить размер) и посмотрите её в "
        "максимально возможном разрешении. Тяжёлый исходник декодируется "
        "только по кнопке (не при каждом действии). Для гигапиксельных панорам "
        "детализация ограничена лимитом памяти."
    )
    ins_x0, ins_y0, ins_x1, ins_y1 = viewer.region_picker(
        base, key="insp_region", bbox=(0.40, 0.40, 0.55, 0.55),
...
with st.expander("Экспертная проверка: отметить и исправить участок (active learning)"):
    st.caption(
        "Выделите прямоугольную область мышью прямо на изображении (потяните "
        "за уголок/край, чтобы изменить размер) и укажите правильный класс. "
        "Исправление сохраняется локально (для будущего дообучения ML)."
    )
    ...
    with ed_prev:
        ...
        corr_x0, corr_y0, corr_x1, corr_y1 = viewer.region_picker(
            base, key="corr_region", bbox=(0.30, 0.30, 0.60, 0.60),
    ...
    if save_corr:
        ...
        if px1 <= px0 or py1 <= py0:
            st.warning("Область пустая — выделите прямоугольник мышью на изображении.")
        else:
